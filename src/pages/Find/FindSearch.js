...
class Index extends Component {
  ...
  componentDidMount() {
      $('head').append('<link rel="stylesheet" href="https://resource.sa-green.cn/kikyo/20181116/css/lightgallery.css">');
  }
  getFeedbackList(page=1){
      this.props.form.validateFields((err, values) => {
          if (!err) {
              ...
              this.props.dispatch({type: 'find/getFindList',data:{beginTimeStr,endTimeStr,groupId,page:page,pageSize:30,status:1}})
          }
      });
  }
  ...
  render() {
      const columns = [
          ...
          {
              title: '内容',
              render : (status,record,index) => <div className="lightgallery list-unstyled">
                  {
          ...
          },
          {
              ...
              render : (status,record,index) => <div>
                  {
                       <Popconfirm title="是否删除?" onConfirm={this.deldteEventStatus.bind(this,record.groupId)} okText="确认" cancelText="取消">
    ...
    return (<PageHeaderWrapper>
      <Card title="发现照片列表查询" className={styles.card} bordered={false}>
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item label="时间段">
                ...
                })(<RangePicker showTime= {{ format: 'HH:mm:ss' } } format = "YYYY-MM-DD HH:mm:ss" style={{
                      width: '100%'
                    }}/>)
                }
              </Form.Item>
        ...
        <Row>
            <Col>
                <Table
                    title={()=><div style={{display:'flex',justifyContent: 'space-between',alignItems: 'center'}}><h3>订单列表 <span style={{fontSize:'12px'}}>共有{this.props.find.recordCount}条数据</span></h3></div>}
