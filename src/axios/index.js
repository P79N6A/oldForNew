...
export const getFeedbackList = data =>
  post({ url: config.QUERY_FEEDBACK_LIST, data }); //查询反馈列表
export const updateEventStatus = data =>
  post({ url: config.UPDATE_EVENT_STATUS, data }); //查询反馈列表
export const deldteEventStatus = data =>
  post({ url: config.DELDTE_EVENT_STATUS, data }); //删除反馈列表
...
export const exportOrderQueryInterval = data =>
  postExport({
